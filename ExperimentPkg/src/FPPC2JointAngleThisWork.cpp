...
JointAngle FunctionThisWork(double u_Boom, double v_Boom, double u_Arm, double v_Arm, double u_Bucket, double v_Bucket)
{
	...
	xy1 = f_uv2xdbarydbarThisWork(u_Boom, v_Boom);
	xy2 = f_uv2xdbarydbarThisWork(u_Arm, v_Arm);
	xy3 = f_uv2xdbarydbarThisWork(u_Bucket, v_Bucket);
	xy4 = f_OnlineDistortThisWork(xy1.X, xy1.Y);
	xy5 = f_OnlineDistortThisWork(xy2.X, xy2.Y);
	xy6 = f_OnlineDistortThisWork(xy3.X, xy3.Y);
	xy7 = f_xibaryibar2XYThisWork(xy4.X, xy4.Y, Z1_ThisWork);
	xy8 = f_xibaryibar2XYThisWork(xy5.X, xy5.Y, Z2_ThisWork);
	xy9 = f_xibaryibar2XYThisWork(xy6.X, xy6.Y, Z3_ThisWork);
	out = f_XY2angleThisWork(xy7.X, xy7.Y, xy8.X, xy8.Y, xy9.X, xy9.Y);

	return out;
}

XY f_uv2xdbarydbarThisWork(double u, double v)
{
	XY out;

	Mat K = (Mat_<double>(3, 3) << fx_ThisWork, 0, u0_ThisWork, 0, fy_ThisWork, v0_ThisWork, 0, 0, 1);
...
XY f_OnlineDistortThisWork(double x, double y)
{
	XY out;
	double k1 = k1_ThisWork;
	double k2 = k2_ThisWork;
	double p1 = p1_ThisWork;
	double p2 = p2_ThisWork;
	double k3 = k3_ThisWork;
	double xi = x;
	double yi = y;
...
XY f_xibaryibar2XYThisWork(double x, double y, double Z)
{
	XY out;

	Mat fai = (Mat_<double>(3, 1) << fai1_ThisWork, fai2_ThisWork, fai3_ThisWork);
	Mat rou = (Mat_<double>(3, 1) << rou1_ThisWork, rou2_ThisWork, rou3_ThisWork);
...
JointAngle f_XY2angleThisWork(double x1, double y1, double x2, double y2, double x3, double y3)
{
	JointAngle out;

	out.Aerfa = atan(y1 / x1) - aerfa1_ThisWork;
	...
	double cosb = (x2 - L1*cos(out.Aerfa)) / lFM2_ThisWork;
	double atanb = atan((y2 - L1*sin(out.Aerfa)) / (x2 - L1*cos(out.Aerfa)));
	if (cosb == 0)
	{
		out.Beita = -0.5*PI - out.Aerfa - beita1_ThisWork;
	}
	else if (cosb > 0)
	{
		out.Beita = atanb - out.Aerfa - beita1_ThisWork;
	}	
	else
	{
		out.Beita = atanb - PI - out.Aerfa - beita1_ThisWork;
	}
	...
	{
		out.Gama = -0.5*PI - out.Aerfa - out.Beita - gama1_ThisWork;
	}
	else if (x3 - L1*cos(out.Aerfa) - L2*cos(out.Aerfa + out.Beita) == 0 && y3 - L1*sin(out.Aerfa) - L2*sin(out.Aerfa + out.Beita) > 0)
	{
		double gamaA = 0.5*PI - out.Aerfa - out.Beita - gama1_ThisWork;
		double gamaB = -1.5*PI - out.Aerfa - out.Beita - gama1_ThisWork;
	...
	else if (x3 - L1*cos(out.Aerfa) - L2*cos(out.Aerfa + out.Beita)!= 0)
	{
		...
		double cosc = (x3 - L1*cos(out.Aerfa) - L2*cos(out.Aerfa + out.Beita)) / lQM3_ThisWork;
		double sinc = (y3 - L1*sin(out.Aerfa) - L2*sin(out.Aerfa + out.Beita)) / lQM3_ThisWork;

		if (cosc > 0 && sinc < 0)
		{
			out.Gama = atanc - out.Aerfa - out.Beita - gama1_ThisWork;
		}
		else if (cosc < 0 && sinc < 0)
		{
			out.Gama = atanc - PI - out.Aerfa - out.Beita - gama1_ThisWork;
		}
		else if (cosc < 0 && sinc>0)
		{
			double gamaA = atanc - PI - out.Aerfa - out.Beita - gama1_ThisWork;
			double gamaB = atanc + PI - out.Aerfa - out.Beita - gama1_ThisWork;
		...
		{
			double gamaA = atanc - out.Aerfa - out.Beita - gama1_ThisWork;
			double gamaB = atanc - 2 * PI - out.Aerfa - out.Beita - gama1_ThisWork;
			if (gamaA >= gamaMin && gamaA <= gamaMax && (gamaB > gamaMax || gamaB<gamaMin))
			{
				out.Gama = gamaA;
			}
